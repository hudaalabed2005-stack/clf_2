...
# ---------------- Roboflow (Hosted Classification) ----------------
ROBOFLOW_API_KEY = os.getenv("ROBOFLOW_API_KEY", "").strip()
RF_HOST   = os.getenv("RF_HOST", "https://classify.roboflow.com").rstrip("/")
PROJECT   = os.getenv("RF_PROJECT", "fresh-or-rotten-detection-1yxeg").strip()  # <-- no trailing spaces
VERSION   = os.getenv("RF_VERSION", "1").strip()

# Full endpoint like: https://classify.roboflow.com/fresh-or-rotten-detection-1yxeg/1
CLASSIFY_URL = f"{RF_HOST}/{PROJECT}/{VERSION}"
...
def extract_top_class(resp_obj):
    """
    Extract {"label":..., "confidence":...%} from Roboflow classification JSON.
    Supports both:
...
# ---------------- /predict (Hosted model only) ----------------
@app.post("/predict")
async def predict(image: UploadFile = File(...)):
    """
    Accept an uploaded image, call Roboflow Classification,
    cache the raw response, and return normalized JSON or a helpful error.
    """
    data = await image.read()

    try:
        # Use both header and query param (some deployments prefer one or the other)
        ...
        url = f"{CLASSIFY_URL}?api_key={ROBOFLOW_API_KEY}"
            ...
            return JSONResponse(
                {
                    ...
                    "hints": [
                        ...
                        f"Confirm model path: {CLASSIFY_URL}",
                        "Ensure the model is deployed for the Hosted API and quota is available."
                    ],
...
class GasReading(BaseModel):
    ...
    vrl: float | None = None
    adc: int | None = None
    adc_max: int | None = 4095      # controller ADC max (e.g., 4095 ESP32, 1023 Arduino UNO)
    vref: float | None = 3.3        # board reference voltage
...
def gas(g: GasReading):
    ...
    """
    VREF = float(g.vref or 3.3)  # <-- fixed missing space bug
    ...
    LAST["gas"] = {"ppm": data["ppm"], "raw": {"vref": VREF, "rl": RL, "r0": r0, "vrl": g.vrl}}
...
def _summarize(last: dict) -> dict:
    ...
    pred = extract_top_class(last.get("vision") or {})
    ...
    # Any class name containing 'rotten' or 'spoiled' => rotten
    model_rotten = bool(pred and isinstance(pred.get("label"), str) and (
        "rotten" in pred["label"].lower() or "spoiled" in pred["label"].lower()
    ))
...
<head>
  ...
  <style>
    ...
    .toast{ position:fixed; right:14px; bottom:14px; background:rgba(17,24,39,.92); color:#fff; padding:10px 14px; border-radius:10px; font-weight:700; box-shadow: 0 10px 30px rgba(0,0,0,.18); transform: translateY(10px); opacity:0; transition: all .2s ease; z-index:30; }
...
<body>
  ...
  <footer>© 2025 Fruit Detector • FastAPI + Hosted Roboflow + MQ-135</footer>
...
const el = {
  ...
  visionBadge:$('visionBadge'), visionTop:$('visionTop'), gasBadges:$('gasBadges'),
  decision:$('decision'), raw:$('raw'), visionStatus:$('visionStatus'),
  gasStatus:$('gasStatus'), toast:$('toast'), chartEmpty:$('chartEmpty'), gasChart:$('gasChart'),
...
function setStatus(which,state){ const s=(which==='vision')?el.visionStatus:el.gasStatus; if(!s) return; const c=(which==='vision')?'#22c55e':'#0ea5e9'; if(state==='busy'){ s.classList.remove('muted'); s.innerHTML=`<span class="dot" style="background:${c}"></span> working…`; } else { s.classList.add('muted'); s.innerHTML=`<span class="dot" style="background:${c}"></span> idle`; } }
...
function updateVisionFromRaw(j){
  if (Array.isArray(j?.predictions) && j.predictions.length) {
    ...
    const lbl = String(pred.class || '?'); const conf = Number((pred.confidence||0)*100).toFixed(1);
    el.visionBadge.style.display='inline-block'; const bad=/rotten|spoiled/i.test(lbl);
    el.visionBadge.className='pill ' + (bad ? 'bad' : 'ok'); el.visionBadge.textContent=`${lbl} • ${conf}%`;
    el.visionTop.textContent=lbl.replace(/_/g,' ').toUpperCase(); return;
  }
  if (j && typeof j==='object' && j.predictions && !Array.isArray(j.predictions)) {
    const items = Object.entries(j.predictions).sort((a,b)=>b[1]-a[1]); if(items.length){
      const [lbl,conf]=items[0]; el.visionBadge.style.display='inline-block'; const bad=/rotten|spoiled/i.test(lbl);
      el.visionBadge.className='pill ' + (bad ? 'bad' : 'ok'); el.visionBadge.textContent=`${lbl} • ${(conf*100).toFixed(1)}%`;
...
async function predictFile(){
  const f = el.file.files[0]; if (!f) { alert('Choose an image'); return; }
  ...
  try {
    ...
    let j = null; try { j = await r.json(); } catch (e) { j = {error:'Bad JSON from server'}; }
...
async function saveSnap(){ const r = await fetch('/cron/snapshot', {method:'POST'}); const j = await r.json(); if(j.ok){ await loadChart(true); } else { alert('No reading to save yet. Send a gas reading first.'); } }

/* Summary */
async function refresh(){
  const r = await fetch('/summary', {cache:'no-store'}); const s = await r.json();
  if (s.vision?.error){ el.visionBadge.style.display='inline-block'; el.visionBadge.className='pill warn'; el.visionBadge.textContent='Vision error'; el.visionTop.textContent=s.vision.error; }
  else if (s.vision && s.vision.label){
    el.visionBadge.style.display='inline-block';
    const lbl = String(s.vision.label); const conf = Number(s.vision.confidence ?? 0).toFixed(1);
    const bad = /rotten|spoiled/i.test(lbl); el.visionBadge.className = 'pill ' + (bad ? 'bad' : 'ok');
    el.visionBadge.textContent = `${lbl} • ${conf}%`; el.visionTop.textContent = lbl.replace(/_/g,' ').toUpperCase();
  } else { el.visionBadge.style.display='none'; el.visionTop.textContent=''; }
  const g  = s.gas_ppm || {}, gf = s.gas_flags || {};
  el.gasBadges.innerHTML = [
    `<span class="pill ${gf.co2_high?'bad':'ok'}">CO₂ ${g.co2??'—'} ppm</span>`,
    `<span class="pill ${gf.nh3_high?'bad':'ok'}">NH₃ ${g.nh3??'—'} ppm</span>`,
    `<span class="pill ${gf.voc_high?'warn':'ok'}">VOC ${g.alcohol??'—'} eq</span>`
  ].join(' ');
